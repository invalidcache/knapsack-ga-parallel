...
__global__ void CudaGaCalculateFitness(Individual *ind, int* objectValues, float *results) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if (i < ind->geneSize) {
        results[i] = ind->genes[i]*objectValues[i];
    }
}
...
__global__ CudaGaCrossOver(Individual *ind1, Individual *ind2, Individual *new_ind, int sep) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if (i < new_ind->geneSize) {
        if (i <= sep)
            new_ind[i] = ind1[i];
        else
            new_ind[i] = ind2[i];
